...
class highlighting(customtkinter.CTkFrame):
    def __init__(self, *args, text, **kwargs):
        ...
        self.highlight_button = customtkinter.CTkCheckBox(
            master=self,
            text=text,
            onvalue="on",
            offvalue="off",
        ...
        self.colour_box = customtkinter.CTkComboBox(
            master=self, values=["Green", "Red"], command=self.set_colour
        )
...
class SearchBox(customtkinter.CTkFrame):
    def __init__(self, *args, columns, tree, **kwargs):
        ...
        self.search_by = customtkinter.CTkComboBox(
            master=self, values=self.columns, command=self.update_default_search
        )
        self.search_by.grid(row=0, column=1, sticky="ns")
        search_entry = customtkinter.CTkEntry(
            master=self, textvariable=self.search_ent_var
        )
    ...
    def filterStudentID(self, *args):
        ...
        for each_item in ItemsTreeView:
            if (
                search
                in (
                    str(self.tree.item(each_item)["values"][self.default_search])
                ).lower()
            ):
...
class TreeViewTable(customtkinter.CTkFrame):
    def __init__(self, *args, items, title, row_size, height, **kwargs):
        ...
        if title is not None:
            title2 = customtkinter.CTkLabel(
                master=self, text="Results", font=self.sub_title_font
            )
            title2.grid(row=0, column=0, sticky="ns")
            self.highlight_gender_button = highlighting(
                master=self, text="Highlight gender"
            )
            self.highlight_gender_button.grid(row=3, column=0)
            self.highlight_location_button = highlighting(
                master=self, text="Highlight origin"
            )
        ...
        self.tree = ttk.Treeview(
            self, columns=self.columns, show="headings", height=self.height
        )
        style = ttk.Style()
    ...
    def file_explorer_saving(self):
        ...
        worksheet.conditional_format(
            "A1:I1000",
            {"type": "formula", "criteria": "=ISODD($H1)", "format": format3},
        )
        workbook.close()
        os.remove(file)
    ...
    def onDoubleClick(self, event):
        ...
        if self.height == 10:
            entry_edit.place(
                x=column_box[0], y=column_box[1] + 28, w=column_box[2], h=column_box[3]
            )
        else:
            entry_edit.place(
                x=column_box[0], y=column_box[1] + 65, w=column_box[2], h=column_box[3]
            )
