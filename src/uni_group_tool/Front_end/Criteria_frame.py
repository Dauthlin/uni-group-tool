...
class CriteriaFrame(customtkinter.CTkFrame):
    def __init__(self, *args, type_to_make, criteria_data, all_data=None, **kwargs):
        ...
        if type_to_make[0] == "diversity":
            ...
            if temp_text == "average":
                CreateToolTip(
                    self.checkbox,
                    "This will attempt to diversify the students by their average score",
                )
            if temp_text == "origin":
                CreateToolTip(
                    self.checkbox,
                    "This will attempt to diversify the students by their origin.\n for example it will try to mix students that are from different origins",
                )
            if temp_text == "gender":
                CreateToolTip(
                    self.checkbox,
                    "This will attempt to diversify the students by their gender score\n for example it will try to mix students that are different genders together ",
                )
            self.label = customtkinter.CTkLabel(
                master=self, text="Priority", width=40, height=25, corner_radius=8
            )
            self.label.grid(row=0, column=1, padx=10, pady=10)
            self.priority = customtkinter.CTkSegmentedButton(
                master=self,
                values=["Very low", "Low", "Medium", "High", "Very high"],
                command=self.priority_update,
                state="disabled",
            )
        ...
        if type_to_make[0] == "types_together":
            ...
            self.label1 = customtkinter.CTkLabel(
                master=self, text=temp_text, width=40, height=25, corner_radius=8
            )
            self.label1.grid(row=0, column=0, padx=10, pady=10)
            self.combobox = customtkinter.CTkComboBox(
                master=self,
                command=self.update_criteria_together,
        ...
        if type_to_make[0] == "groups":
            self.groups = customtkinter.CTkSegmentedButton(
                master=self,
            ...
            self.value = customtkinter.CTkComboBox(
                master=self,
    ...
    def priority_update(self, data):
        for i, j in zip(
            (0.2, 0.4, 0.6, 0.8, 1), ["Very low", "Low", "Medium", "High", "Very high"]
        ):
    ...
    def update_criteria_together(self, data):
        if self.combobox.get().isnumeric():
            self.criteria_data.set_should_be_together(
                self.type_to_make[1][0], self.type_to_make[1][1], self.combobox.get()
            )
