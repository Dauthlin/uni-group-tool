...
class Window(tk.Tk):
    def __init__(self, loop):
        ...
        button_block = tk.Button(
            text="Calculate Sync", width=10, command=self.calculate_sync
        )
        button_block.grid(row=2, column=0, sticky=tk.W, padx=8, pady=8)
        button_non_block = tk.Button(
            text="Calculate Async",
            width=10,
            command=lambda: self.loop.create_task(self.calculate_async()),
        )
    ...
    async def execute(self, json_data):
        popen = subprocess.Popen(
            [sys.executable, "Run_app.py", json.dumps(json_data)],
